...
/* The head's own call to action, on the pages that carry one. Gold on the
   band for the same reason the showcase hero's is: the primary colour is
   the band, so a primary-coloured button on it would not be a button. */
.rx-page-head--metro .rx-cta-row{justify-content:center;margin-top:var(--rx-lg)}
.rx-page-head--metro .rx-btn-wa{background:var(--rx-accent);color:var(--rx-ink);box-shadow:none}
...
/* ---- hero layout: centred (Modern) ----
   A full-bleed, inverted band — WHETHER OR NOT a photo exists. The modifier
   class lives on the SECTION (.rx-pharmacy-hero), not on .rx-hero-inner —
   that section already paints a full-bleed background (the light tint every
   other layout uses), so the override has to sit at the same level to
   replace it rather than colour just the centred, padded-in column a level
   deeper and leave the tint showing all around it as an unintended frame.
   .rx-pharmacy-hero.rx-hero--centered, not just .rx-hero--centered, so this
   wins on specificity rather than on source order alone. The photo (if any)
   is a dimmed cover background, painted by DOM order beneath the ::after
   scrim rather than by an explicit z-index war, with the copy lifted onto its
   own stacking context so it always ends up on top regardless. */
.rx-pharmacy-hero.rx-hero--centered{position:relative;overflow:hidden;background:var(--rx-primary);color:var(--rx-on-primary)}
.rx-hero-bg{position:absolute;inset:0;width:100%;height:100%;object-fit:cover}
.rx-pharmacy-hero.rx-hero--centered::after{content:"";position:absolute;inset:0;background:var(--rx-primary);opacity:.76}
.rx-hero--centered .rx-hero-copy{position:relative;z-index:1;max-width:46rem;margin-inline:auto;text-align:center}
.rx-hero--centered .rx-eyebrow,
.rx-hero--centered h1,
.rx-hero--centered .rx-lede{color:var(--rx-on-primary)}
.rx-hero--centered .rx-eyebrow{opacity:.86}
.rx-hero--centered h1{font-size:clamp(2.4rem,1.6rem + 3.6vw,4.4rem)}
.rx-hero--centered .rx-lede{opacity:.86;margin-inline:auto}
.rx-hero--centered .rx-cta-row{justify-content:center}
.rx-hero--centered .rx-btn-wa{background:var(--rx-on-primary);color:var(--rx-primary);box-shadow:none}
.rx-hero--centered .rx-btn-ghost{box-shadow:inset 0 0 0 1px color-mix(in oklab, var(--rx-on-primary) 55%, transparent);color:var(--rx-on-primary)}
.rx-hero--centered .rx-btn-ghost:hover{background:color-mix(in oklab, var(--rx-on-primary) 16%, transparent);box-shadow:inset 0 0 0 1px var(--rx-on-primary);color:var(--rx-on-primary)}

/* ---- hero layout: framed (Premium) ----
   An asymmetric split with the photo inset in its own frame rather than
   bleeding to the card's edge — a quieter, more considered arrangement than
   the even diptych "split" uses. */
.rx-hero--framed .rx-hero-media{padding:var(--rx-sm);background:var(--rx-tint);border-radius:var(--rx-radius)}
...
/* NOT margin:0 — that was fine while .rx-panel only ever held one paragraph
   (an FAQ answer, where the existing p:last-child rule already zeroes the
   trailing margin), but this class is now also used for prose sections with
   several paragraphs (a health-article section, a service's two-part
   description), and margin:0 there ran every paragraph into the next with no
   gap between them. */
.rx-panel p{color:var(--rx-muted)}
...
/* ---- pharmacist CTA: statement variant (Premium) ----
   A quieter, more considered close than the plain paragraph — a tinted band
...
/* No breadcrumb styles: the visible trail was removed from the generated
   ...
   describes the real URL hierarchy rather than a thing on the page.
   (Careful with the schema type's name here — this sheet is inlined into
   every document, so a word written in a comment is a word a test grepping
   the HTML will find. Naming it cost a green suite once.) */
...
@media (min-width:768px){
  ...
  /* One column, whatever the width: there is no second thing to put in it —
     "centered"'s background photo is absolutely positioned, not a grid item,
     so without this override a single grid child would be stretched into
     just the FIRST of the two columns above rather than spanning the centre. */
  .rx-hero-inner.rx-hero--type,
  .rx-hero--centered .rx-hero-inner{grid-template-columns:minmax(0,1fr)}
...
@media (prefers-reduced-motion:no-preference){
  @supports (animation-timeline:view()){
    .rx-block{
      ...
      /* Runs across most of the section's entry rather than finishing near
         the top of the viewport, so the motion plays out over an ordinary
         scroll gesture instead of resolving before the reader has caught it —
         found live: at the old, shorter range it read as "no animation" even
         though one was running. */
    ...
    /* One step finer inside the sections that are a set of things rather than
       ...
       .rx-service-grid matters most here — seven stacked cards on a phone can
       run 3000px tall, and the single coarse rise on .rx-block below is
       computed against the WHOLE section's own entry, so on something that
...
/* MOVES, NEVER FADES — a correctness decision, not a taste one.
   ...
   pharmacy's website as a blank page. That happened here twice, so opacity
   stays out regardless of how the movement itself is tuned: the worst case
   for a transform-only reveal is content sitting visibly low, which is
   always readable, never blank.
   36px and a touch of scale, not 14px translate alone — the smaller version
   was correct but reads as "nothing is happening" at a normal scroll speed,
   which was reported directly. Nothing about the reveal carries information,
   so making it larger costs nothing a reduced-motion visitor would miss —
   they never see it at all, per the query below.
